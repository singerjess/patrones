...
def main():
    ...
    output_pattern_subtraction_optimized(base_patterns, pattern_mapper, GraphMapper(), pattern_unifying,
                                         two_thinness_patterns, subpattern_calculator)


def merge_resulting_graphs(pattern_unifying: PatternUnifying, patterns: List[Pattern], base_pattern: Pattern):
    ...
    pattern_induced_graphs = [expanded_pattern_graph_mapper.map(pattern) for pattern in decided_patterns]
    for index, graph in enumerate(pattern_induced_graphs):
        for another_index, another_graph in enumerate(pattern_induced_graphs):
            if index != another_index:
                if graph.is_subgraph_of(another_graph):
                    pattern_induced_graphs.remove(another_graph)
...
def output_pattern_subtraction_optimized(base_patterns: List[Pattern],
    ...
    for base_pattern in base_patterns:
        patterns_result_file_name = "resources/results/patterns/base_pattern_" + str(index) + ".json"
        ...
        with open(graphs_result_file_name, 'w') as f:
            f.write(graph_mapper.map_graphs_to_json(merge_resulting_graphs(pattern_unifying, results, base_pattern)))
