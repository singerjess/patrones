...
class SubgraphCalculator:
    ...
    def is_induced_subgraph(self, nodes_subgraph: int, edges_subgraph: List[Edge],
                            nodes_supergraph: int, edges_supergraph: List[Edge]):
        if nodes_supergraph < nodes_subgraph:
            return False
        possible_assignments = self._injective_node_mapper.possible_assignments_unordered(
            nodes_supergraph, nodes_subgraph)
